...
bin/nextflow src/pipeline.nf -c src/pipeline.config -profile local --fastq "data/examples/tiny_dataset/fastq/*tiny_R{1,2}.fastq" --fasta "data/examples/tiny_dataset/fasta/tiny_v2.fasta" --annot "data/examples/tiny_dataset/annot/tiny.gtf" --todo "fastqc+cutadapt+urqt+fastqc+multiqc+split_ref+bowtie2+rsem"
