...
for (i in seq_along(analysis)){
  ...
  write.table(as.data.frame(resLFC), file = paste(save.path.dir, "/deseq2_shrinked_analysis_", save.dir, ".txt", sep = ""), sep = "\t", col.names = T, row.names = F, quote = F)
  ...
  if(do.perm.dist.tRNA){
    ...
    for (w in c(1000, 10000, 25000, 50000, 100000)) {
      print(w)
      dist.clust.obs <- list()
    ...
    output_perm_density_feature <- data.frame(N = rep(N, length(pval)), w = c("1kb", "10kb", "25kb", "50kb", "100kb"), pval = pval, obs.clust = obs, stringsAsFactors = F)
  ...
  pdf(paste(save.path.dir, "/density_up_down_reg_", save.dir, ".pdf", sep = ""), w = 7*length(chr.diff.all), h = 7)
  den.d <- NULL; pos.d <- NULL; den.nd <- NULL; pos.nd <- NULL; ud <- c(); equ <- c()
  for (chr in chr.diff.all) {
  ...
  abline(h=0, col = "gray")
  text(offset[2:length(offset)], rep(1, length(offset)-1), paste(ud, " up/downregulated", sep = ""), col = "darkorchid4", cex =0.8)
  ...
  glm.nb.dist.tRNA <- glm.nb(distance.tRNA ~ group, data = df.dist.tRNA)
