...
results$signif <- ifelse(results$padj_rrp6D < 0.05 & results$padj_cut14 < 0.05, "cut14-208 & rrp6D", "no")

results$signif <- as.factor(results$signif)
# graph on log2FoldChange

model <- lm(data = results, log2FoldChange_rrp6D ~ log2FoldChange_cut14)
summary(model)
rmodel <- MASS::rlm(data = results, log2FoldChange_rrp6D ~ log2FoldChange_cut14)
summary(model)
model$coefficients
rmodel$coefficients
table(results$signif)
...
p <- ggplot() +
  ...
  geom_abline(
    slope = rmodel$coefficients[2],
    intercept = rmodel$coefficients[1],
    col="#F8766D"
  ) +
  theme_bw() +
  labs(
